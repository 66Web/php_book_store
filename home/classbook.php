<?php
  ...
  //图书分页显示
  $pagesize = 5;
  $sqlBook_page = "select * from book where class_id = {$id}";
		...
		<div class="content">
			<div class="floor">
                ...
                <?php
                    ...
                    if($page>1 && $totalpage>1){
                       echo "<a href=?page=".($page-1)."&class_id={$rowClass['id']}>上一页&nbsp;&nbsp;</a>";
                    }
                    if($page>=1 && $totalpage>$page){
                       echo "<a href=?page=".($page+1)."&class_id={$rowClass['id']}>下一页&nbsp;&nbsp;</a>";
                    }
                    if($page>=1 && $totalpage>1){
                       echo "<a href=?page=".$totalpage."&class_id={$rowClass['id']}>尾页</a>";
                    }
                    echo "</div>";
                ?>
